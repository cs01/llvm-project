...
-fnullability-safety, and measures the compile-time delta with
proper statistical rigor: warmup runs, multiple iterations, confidence
intervals, and paired significance testing.

Usage:
    python3 nullability-safety-benchmark.py --clang-binary /path/to/clang
    python3 nullability-safety-benchmark.py --clang-binary /path/to/clang --iterations 20
"""
...
def analyze_trace(trace_path):
    ...
    event_map = {
        "NullabilitySafetyAnalysis": "analysis_us",
...
def generate_report(all_results, args):
    ...
    lines.append("`-fnullability-safety -fnullability-default=nullable`. Warmup runs")
    ...
    lines.append("")
    lines.append("`-ftime-trace` isolates time spent inside `NullabilitySafetyAnalysis` from")
    ...
    lines.append("")
    lines.append("The `-ftime-trace` instrumentation isolates `NullabilitySafetyAnalysis` time:")
    lines.append("")
    ...
    lines.append("4. **The analysis is opt-in** — zero cost if `-fnullability-safety` is not passed")
    lines.append("")
