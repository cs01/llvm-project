#!/usr/bin/env python3
"""
Clang Static Analyzer vs nullability-safety performance comparison.

Measures wall-clock time for:
  1. Clang Static Analyzer (--analyze, core.NullDereference)
  2. Flow-sensitive nullability (-fnullability-safety)
  3. Baseline compilation (no analysis)

Both tools find null dereferences, but CSA uses symbolic execution
(path-sensitive, interprocedural) while nullability-safety uses forward
dataflow on the CFG (intraprocedural, like -Wthread-safety).

Usage:
    python3 nullability-safety-csa-benchmark.py \
...
def generate_null_deref_code(n):
    ...
    nullability-safety can analyze. Uses _Nullable annotations so
    nullability-safety is exercised, and straightforward pointer patterns
...
def generate_loop_code(n):
    ...
    while nullability-safety uses fixpoint convergence.
    """
    code = """
struct ListNode {
    int val;
...
def main():
    parser = argparse.ArgumentParser(
        description="CSA vs nullability-safety performance comparison.")
...
def generate_report(all_results, args, workloads):
    ...
    lines.append("| Invocation | Separate step (`--analyze`) | "
                 "Part of compilation (`-fnullability-safety`) |")
    ...
    lines.append("CSA is better suited as a periodic deep-analysis tool; "
                 "nullability-safety is better suited")
