...
cost of flow-sensitive nullability vs other CFG-based Sema analyses:
  ...
  4. -fnullability-safety -fnullability-default=nullable
...
The key insight this benchmark provides: if nullability-safety adds ~2%
on top of thread-safety's ~15%, that's inarguable for reviewers.

Usage:
    python3 nullability-safety-cross-analysis-benchmark.py \
...
def generate_nullability_annotated_code(n):
    ...
    nullability-safety analysis specifically.
    """
    code = """
struct Obj {
    int value;
...
def generate_report(all_results, args, n_values):
    ...
    lines.append("2. **The analysis is opt-in** — zero cost when `-fnullability-safety`")
