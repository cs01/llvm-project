# Nullability Safety Architecture
...
graph TB
    subgraph INPUT["🔧 User Input"]
        FLAGS["-fnullability-safety<br>-fnullability-default=nullable"]
        PRAGMA["#pragma clang assume_nonnull"]
        SOURCE["Source code with pointers"]
    end

    subgraph LAYER3["🚦 Layer 3: Gating — SemaDecl.cpp"]
        GATE["ActOnStartOfFunctionDef()<br>Sets NullabilitySafetyEnabled"]
    end

    subgraph LAYER2["🔌 Layer 2: Glue — AnalysisBasedWarnings.cpp"]
        SHOULD["shouldEnableNullabilitySafety()"]
        CFG_BUILD["Build CFG with<br>setAllAlwaysAdd()"]
        REPORTER["NullabilitySafetyReporter<br>implements NullabilitySafetyHandler"]
        DIAG["S.Diag() → compiler warning"]
    end

    subgraph LAYER1["🧠 Layer 1: Analysis Engine — NullabilitySafety.cpp"]
        ENTRY["runNullabilitySafetyAnalysis()"]
...
The core fixpoint iteration loop (`runNullabilitySafetyAnalysis`, lines 880–1009).
...
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '14px'}}}%%
flowchart TD
    FUNC["📄 Function definition encountered"] --> CHECK_FLAG{"-fnullability-safety<br>enabled?"}

    CHECK_FLAG -->|no| OFF["🚫 Analysis disabled<br>Zero overhead"]
    CHECK_FLAG -->|yes| CHECK_DEFAULT{"-fnullability-default=?"}

    CHECK_DEFAULT -->|"nullable or nonnull"| ENABLED["✅ NullabilitySafetyEnabled = true<br>Analysis runs on this function"]
...
graph LR
    subgraph L1["🧠 Layer 1 — Analysis Engine"]
        H["NullabilitySafety.h<br><i>Handler interface + entry point</i>"]
        CPP["NullabilitySafety.cpp<br><i>~1010 lines: worklist, transfer fns,<br>condition analysis, edge states</i>"]
    end

    subgraph L2["🔌 Layer 2 — Sema Glue"]
        ABW["AnalysisBasedWarnings.cpp<br><i>NullabilitySafetyReporter,<br>shouldEnable, CFG build</i>"]
        SEMA_H["Sema.h<br><i>NullabilitySafetyEnabled flag</i>"]
    ...
    subgraph INFRA["⚙️ Flags & Diagnostics"]
        ...
        DIAG_GROUPS["DiagnosticGroups.td<br><i>-Wnullability-safety-dereference</i>"]
    end

    subgraph TESTS_GRP["🧪 Tests"]
        TESTS["test/Sema/nullability-safety-*.cpp/.c<br><i>28+ lit tests</i>"]
    end
...
graph TB
    ...
    subgraph FN_GRP["🛡️ NullabilitySafety"]
        FN["NullabilitySafety.cpp"]
        FN_H["NullabilitySafetyHandler"]
        FN_R["NullabilitySafetyReporter"]
    end
...
**Key difference**: ThreadSafety and UninitializedValues use **per-block** state. NullabilitySafety uses **per-edge** state for more precise branch refinement — a technique from the dataflow analysis literature known as "edge-based dataflow" (similar to conditional constant propagation).
