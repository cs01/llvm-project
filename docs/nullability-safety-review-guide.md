# Nullability Safety Architecture Review Guide

A written walkthrough of the flow-sensitive null pointer dereference checker for code reviewers and contributors.

See also: [nullability-safety-architecture.md](nullability-safety-architecture.md) for Mermaid diagrams of the same concepts.
...
**`clang/lib/Analysis/NullabilitySafety.cpp`** (~1,100 lines)
...
#### The core loop (`runNullabilitySafetyAnalysis`)
...
- **`NullabilitySafetyReporter`** — implements the `NullabilitySafetyHandler` callback interface. When the analysis calls `handleNullableDereference()`, this converts it into a `S.Diag()` call (Clang's diagnostic emission). That's what produces the actual compiler output:
...
- **`shouldEnableNullabilitySafety`** — checks whether analysis is enabled for this specific function (via the `NullabilitySafetyEnabled` flag set in Layer 3).

- **CFG build integration** (lines 3222–3231): When nullability-safety is enabled, it requests `setAllAlwaysAdd()` on the CFG builder, which means ALL statements become visible as CFGElements. Without this, some statements get optimized away in the CFG. For example:
...
Sets `NullabilitySafetyEnabled` per-function in `ActOnStartOfFunctionDef`. The analysis only fires when:
...
clang -fnullability-safety -fnullability-default=nullable foo.c
```
...
|------|----------------|
| `DiagnosticSemaKinds.td` | Warning `warn_nullability_safety_dereference` ("dereference of nullable pointer") + fix note `note_nullable_dereference_fix` |
| `DiagnosticGroups.td` | `-Wnullability-safety-dereference` (the warning), `-Wnullability-safety` (parent group) |
...
|------|---------|---------|
| `-fnullability-safety` | Master switch (LangOpt) | `clang -fnullability-safety foo.c` |
...
**Key difference**: ThreadSafety and UninitializedValues use per-block state. NullabilitySafety uses **per-edge state** for more precise branch refinement. This is a valid and well-known technique in dataflow analysis literature — it's called "edge-based dataflow" or "conditional constant propagation style" tracking.
...
| File | Role |
|------|------|
| `include/clang/Analysis/Analyses/NullabilitySafety.h` | Public API (handler + entry point) |
| `lib/Analysis/NullabilitySafety.cpp` | The analysis (~1,100 lines) |
...
| `include/clang/Sema/Sema.h` | `NullabilitySafetyEnabled` flag |
...
| `test/Sema/nullability-safety-*.cpp/.c` | 38 lit tests |
