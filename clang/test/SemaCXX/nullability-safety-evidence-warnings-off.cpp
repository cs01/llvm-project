...
// diagnostics. With every flow warning group suppressed (-Wno-nullability-safety)
...
// -Wno-nullability-safety skipped the analysis entirely and no remark fired.
//
// Evidence remarks fire; no warnings (they are all suppressed) -> -verify only
// sees the expected remarks.
//
// RUN: %clang_cc1 -fsyntax-only -fnullability-safety -fnullability-default=nullable -Wno-nullable-to-nonnull-conversion -Wno-nullability-safety -std=c++17 -Rnullsafe-evidence %s -verify
...
int deref(int *_Nullable p) {
  return *p; // no warning: -Wno-nullability-safety
}
