// nullability-safety-analysis.cpp - Core flow-sensitive nullability analysis tests.
//
...
//
// RUN: %clang_cc1 -fsyntax-only -fnullability-safety -fnullability-default=nullable -Wno-nullable-to-nonnull-conversion -std=c++11 -fcxx-exceptions -Wno-unused-value %s -verify
// RUN: %clang_cc1 -fsyntax-only -fnullability-safety -fnullability-default=nullable -Wno-nullable-to-nonnull-conversion -std=c++17 -fcxx-exceptions -Wno-unused-value %s -verify
// RUN: %clang_cc1 -fsyntax-only -fnullability-safety -fnullability-default=nullable -Wno-nullable-to-nonnull-conversion -std=c++20 -fcxx-exceptions -Wno-unused-value %s -verify
...
// ===----------------------------------------------------------------------===//
// Flow taint on a _Nonnull variable is judged the same at every site
// ===----------------------------------------------------------------------===//
// A _Nonnull local assigned null is nullable at the argument and return sites
// too, not only at the initializer and at a dereference.
