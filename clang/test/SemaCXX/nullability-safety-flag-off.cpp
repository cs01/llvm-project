// Flag-off no-op coverage. Without -fnullability-safety and without
// -fnullability-default, representative nullability-safety test cases must
...
// With -fnullability-safety these all warn (nullable dereference);
...
int *_Nonnull nullInitNonnull() {
  int *_Nonnull q = nullptr; // flow-on: warn_nullability_safety_null_init; off: silent
  return q;
}
