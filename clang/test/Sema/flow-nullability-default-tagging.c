// RUN: %clang_cc1 -fsyntax-only -Wno-error=int-conversion -fflow-sensitive-nullability -I %S/Inputs %s -verify=plain
// RUN: %clang_cc1 -fsyntax-only -Wno-error=int-conversion -fflow-sensitive-nullability -fnullability-default=nonnull -DPARTIAL_HEADER -I %S/Inputs %s -verify=plain
// RUN: %clang_cc1 -fsyntax-only -Wno-error=int-conversion -fflow-sensitive-nullability -fnullability-default=nullable -DPARTIAL_HEADER -I %S/Inputs %s -verify=tagged
...
#include "flow-nullability-partially-annotated.h"
#endif
