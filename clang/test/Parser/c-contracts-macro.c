...
// RUN: %clang_cc1 -fsyntax-only -fc-contracts -verify=expected,cmdline -Densures=oops %s
...
#define requires(x) // expected-warning {{macro named 'requires' hides the contract clause keyword of the same name}}
...
// cmdline-warning@* {{macro named 'ensures' hides the contract clause keyword of the same name}}
