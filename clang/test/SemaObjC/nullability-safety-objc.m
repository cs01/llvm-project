...
// QualType::isPointerType() (NullabilitySafety.cpp), which is TRUE only for C/C++
...
//     ObjCPropertyRefExpr / ObjCIvarRefExpr handler in NullabilitySafety.cpp).
//   - 'NSError **' is a raw PointerType at its OUTER level, so the C-pointer
//     deref path DOES fire on it.
//
// RUN: %clang_cc1 -fsyntax-only -fobjc-arc -fnullability-safety -fnullability-default=nullable -Wno-unused-value %s -verify
...
void property_access_on_nullable(Widget * _Nullable w) {
  ...
  // NullabilitySafety.cpp). Should narrow/track once isObjCObjectPointerType is
  // handled.
}
